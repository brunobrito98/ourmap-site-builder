...
const Features = () => {
  return (
    <section id="features" className="py-32 bg-gradient-to-b from-background via-secondary/10 to-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center max-w-4xl mx-auto mb-20 space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 backdrop-blur-glass mb-4">
            <span className="text-sm font-bold text-primary tracking-wide uppercase">Recursos</span>
          </div>
          <h2 className="text-5xl md:text-6xl lg:text-7xl font-bold text-foreground leading-tight tracking-tight text-balance">
            Tudo Que Você Precisa
            <span className="block mt-3 bg-clip-text text-transparent" style={{ backgroundImage: 'var(--gradient-hero)' }}>Em Um Só Lugar</span>
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8 lg:gap-10 max-w-7xl mx-auto">
          {features.map((feature, index) => (
            <div
              key={index}
              className="group relative bg-card/80 backdrop-blur-glass rounded-3xl p-8 lg:p-10 shadow-lg hover:shadow-[var(--shadow-elegant)] transition-all duration-500 hover:-translate-y-3 border border-border/50 hover:border-primary/30"
            >
              {/* Icon */}
              <div className={`mb-8 w-20 h-20 rounded-2xl ${index === 1 ? 'bg-gradient-to-br from-[hsl(var(--pink))]/20 to-[hsl(var(--pink))]/5 shadow-[var(--shadow-pink)]' : index === 2 ? 'bg-gradient-to-br from-purple-500/20 to-purple-500/5' : 'bg-gradient-to-br from-primary/20 to-primary/5 shadow-[var(--shadow-elegant)]'} flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-all duration-500`}>
                <feature.icon className={`w-10 h-10 ${index === 1 ? 'text-[hsl(var(--pink))]' : index === 2 ? 'text-purple-500' : 'text-primary'}`} />
              </div>

              {/* Feature Image */}
              <div className="mb-8 rounded-2xl overflow-hidden bg-gradient-to-br from-primary/5 to-primary/10 p-6 group-hover:scale-105 transition-transform duration-500">
                <img
                  src={feature.image}
